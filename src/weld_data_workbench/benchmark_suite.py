...
def _run_preview_benchmark(
    ...
    if not selected:
        return {"enabled": False, "reason": "no samples selected", "requested_samples": sample_limit}
    ...
    latencies = {modality: [] for modality in _MODALITIES}
    files_written = {modality: 0 for modality in _MODALITIES}
    warnings = {modality: [] for modality in _MODALITIES}
    cold: list[float] = []
    warm: list[float] = []
    cold_files = 0
    warm_files = 0
        ...
        for modality in _MODALITIES:
            ...
            try:
                elapsed, result = _timed(
                    lambda modality=modality, target=target, sample=sample: _preview_one_modality(
                        preview_config, sample, modality, target
                    )
                )
            ...
            latencies[modality].append(elapsed)
            files_written[modality] += count
            warnings[modality].extend(f"{sample_id}: {message}" for message in messages)

        elapsed, bundle = _timed(lambda sample_id=sample_id: generator.generate(sample_id, force=True))
        cold.append(elapsed)
        cold_files += len(bundle.generated_files)
        elapsed, bundle = _timed(lambda sample_id=sample_id: generator.generate(sample_id, force=False))
        warm.append(elapsed)
...
def _run_feature_benchmark(
    ...
    return {"enabled": True, "requested_samples": sample_limit, "workers": workers, "modalities": results}
...
def _optional_stage(name: str, operation: Callable[[], dict[str, Any]], warnings: list[str]) -> dict[str, Any]:
    try:
        return operation()
    except Exception as exc:
    ...
    *,
    ...
    try:
        ...
        return BenchmarkSuiteReport(
            ...
            features=features,
            api=api,
            scratch={"path": str(root), "retained": selected.keep_scratch, "auto_owned": auto_owned},
            warnings=warnings,
        )
    finally:
